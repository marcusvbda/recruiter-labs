...
/**
 * The identifier-redaction rules shared by every sanitizer that prepares
 * candidate-submitted material for an AI agent.
 *
 * It lives in one place on purpose: the per-application evaluation context and
 * the candidate-level sourcing context must remove exactly the same identifiers,
 * in exactly the same order, or one of them would quietly leak what the other
 * removes.
 *
 * **What this is not.** It is not anonymisation, it does not eliminate bias, and
 * it makes no legal fairness guarantee. It removes direct identifiers with no
 ...
 * identifiers are excluded from the AI context.
 *
 ...
 * accomplishments and numbers are evidence. There is no protected-characteristic
 * detection here, and no inference of race, gender or ethnicity.
 */
trait RedactsCandidateIdentifiers
{
     ...
     */
    protected function identifierPatternsFor(?Candidate $candidate): array
    {
        $patterns = [];
     ...
     */
    protected function redactIdentifiers(?string $value, array $patterns, int &$redactions): ?string
    {
