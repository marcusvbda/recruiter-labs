...
- `.claude/skills/evaluation-integrity/SKILL.md` — AI agents, candidate evaluation,
...
- **Candidate Evaluation is contextual evidence, not the primary workflow
  ...
  tracks. Show the evaluation once, inside the Review tab (stage first, then
  Fit and Evidence Coverage as two distinct figures and what still needs
  validation); never repeat it in a second card, and keep the Interview Brief
  detail only under Interviews.
...
- **Attention leads the layout, not just the reading order.** On desktop it takes
  the wide column with the compact "RecruiterLabs AI now" region beside it; on
  narrow screens they stack with attention first. The regions then answer the
  remaining questions in order: what the AI finished for the workspace in one
  stated recent period, then the recruiter's agenda and the active hiring
  processes.
- **The AI regions summarise services, never reimplement them.** "AI now" is a
  capped read of `AiActivityService` that opens the same AI Activity panel the
  topbar owns (never a second panel), and it refreshes on the workspace's
  `ai_activity_{slug}` channel rather than on a timer. Completed-work figures
  come from `RecruiterProductivityService` and count only automatic, still
  valid, in-period work; "estimated review time saved" renders only when the
  workspace configured a manual-review baseline and always says it is an
  estimate.
...
- **Regions cap what they list and say what they left out.** Presentation caps
  live in the page (`AttentionLimit`, `AgendaLimit`, `ProcessLimit`,
  `AiActivityLimit`) and the
