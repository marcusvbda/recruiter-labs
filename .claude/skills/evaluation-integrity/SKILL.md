...
description: Rules for everything AI-facing in the product — App\Ai\Agents conventions (structured output, TOON context, token efficiency), criteria extraction and automatic activation, candidate evaluation and its revisions, stale evaluations, context sanitization, evidence provenance, fit/coverage/confidence semantics, interview brief, referral neutrality and AI cache/schema versioning. Load before touching any of them.
---
...
- job criteria extraction and automatic activation
  (`ExtractJobCriteria`, `ReplaceJobCriteria`, `RequireJobCriteriaReview`,
  `ReleaseApplicationsForCurrentCriteria`);
...
- `.claude/skills/recruitment-workflow/SKILL.md` — where evaluation output is
  displayed (Kanban, application workspace, Overview) and the workflow rules
  that own those surfaces.
- `CLAUDE.md` — global rules (language, Git, testing, documentation).
...
- **AI prepares evaluation criteria; they become current automatically and a
  human owns them by editing.** A successful extraction (`ReplaceJobCriteria`)
  makes the criteria current at once: status `Completed`, with
  `criteria_confirmed_generation` / `criteria_confirmed_at` recording the
  activated revision and `criteria_confirmed_by_id` left null (system-attributed).
  There is no human confirmation step and no `ConfirmJobCriteria`. The criteria
  stay clearly labelled AI-generated and editable, a saved edit is authoritative
  immediately, and Rebuild stays available. Missing context, failure and
  allowance blocks activate nothing — never invent criteria.
  `JobCriteriaProcessingStatus::AwaitingReview` survives only as a legacy state
  for revisions stored before automatic activation.
- **Candidate evaluation cannot run against criteria that are not current.** The
  single gate is `Job::hasConfirmedCriteria()` — the name is historical, it means
  "has current criteria": status *and* an activated revision equal to
  `criteria_generation` — and both `ScheduleApplicationFitAnalysis` and
  `AnalyzeApplicationFit` check it, the second because criteria can change
  between queueing and running. Until then the application waits in
  `ApplicationAnalysisStatus::AwaitingCriteria`, and activating criteria releases
  it (`ReleaseApplicationsForCurrentCriteria`) through the existing queue, quota
  and BYOK path. Never bypass those actions.
- **An evaluation belongs to two exact revisions.** The Application analysis
  generation identifies the evaluation request, and the current Job criteria
...
- **The stored criteria revision identifies the evaluation that produced it.**
  ...
  in-flight extraction and re-points the activation record at the new revision
  (criteria kept, current immediately) in one write.
- **A stale evaluation is never presented as current.** Scores are not deleted —
  ...
  Review tab, Kanban card, candidate view) goes through
  `hasCurrentEvaluation()`, and the Review tab renders an explicit
  `outdated` state instead. A new current revision reschedules in-process
  applications automatically; terminal ones keep their historical evaluation.
- **Only evaluation-relevant job changes advance the criteria revision.** Title,
  ...
  `RequireJobCriteriaReview` (its name is historical: it now applies the new
  revision and keeps the criteria) — never increment on every edit, or unrelated
  saves would needlessly re-evaluate every active application and burn AI
  allowance.
