...
decision recorded only in `.claude/state/<feature>.md`.
...
Global rules in `CLAUDE.md` always win: English only, no branches, commits or
...
2. Load the domain skill(s) the feature touches:
   - recruiter surfaces → `.claude/skills/recruitment-workflow/SKILL.md`;
...
4. Compute a deterministic SHA-256 identity for `spec.md`. Record either the
   ...
   `.claude/state/<feature>.md` if it exists. Repository inspection also checks
...
Create `.claude/state/<feature>.md` (and `.claude/state/` if needed) with this minimum
shape:
...
`.claude/skills/project-core/SKILL.md`:
...
Expect the reviewer format from `.claude/agents/code-reviewer.md`: per-criterion
...
clean. Apply `.claude/skills/project-core/SKILL.md`'s "Token discipline" loop caps
...
- **APPROVED** → mark the task `DONE` in `.claude/state/<feature>.md`, record its
...
Retain `.claude/state/<feature>.md` unless the user asks to delete it; it is
