<x-filament::section
    :heading="__('candidates.materials.heading')"
    :description="__('candidates.materials.description')"
    icon="heroicon-o-document-text"
>
    @if ($importOrigin)
        <div class="mb-3 flex flex-wrap items-center gap-x-2 gap-y-1 border-b border-gray-100 pb-3 text-sm text-gray-600 dark:border-white/5 dark:text-gray-400">
            <span class="font-medium text-gray-700 dark:text-gray-300">{{ __('candidates.materials.import_origin') }}:</span>
            <span>
            ...
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div class="min-w-0">
                    <p class="truncate font-medium text-gray-950 dark:text-white">
                        ...
                        <span class="font-normal text-gray-500 dark:text-gray-400">({{ strtoupper($material['extension']) }} · {{ $material['size'] }})</span>
                    </p>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @if ($material['view_url'])
                        <x-filament::link :href="$material['view_url']" icon="heroicon-m-eye" icon-position="after" size="sm" target="_blank">
                            {{ __('candidates.materials.view') }}
                        </x-filament::link>
                    @endif

                    @if ($material['download_url'])
                        <x-filament::link :href="$material['download_url']" icon="heroicon-m-arrow-down-tray" icon-position="after" size="sm">
                            {{ __('candidates.materials.download') }}
                        </x-filament::link>
                    @endif

                    <x-filament::button size="sm" color="gray" outlined wire:click="mountAction('correctMaterialMetadata', { material: {{ $material['id'] }} })">
                        {{ __('candidates.materials.actions.correct_metadata') }}
                    </x-filament::button>

                    @if ($material['can_retry'])
                        <x-filament::button size="sm" color="gray" outlined wire:click="mountAction('retryMaterialPreparation', { material: {{ $material['id'] }} })">
                            {{ __('candidates.materials.actions.retry') }}
                        </x-filament::button>
                    @endif

                    <x-filament::button size="sm" color="gray" outlined wire:click="mountAction('archiveMaterial', { material: {{ $material['id'] }} })">
                        {{ __('candidates.materials.actions.archive') }}
                    </x-filament::button>

                    <x-filament::button size="sm" color="danger" outlined wire:click="mountAction('deleteMaterial', { material: {{ $material['id'] }} })">
    ...
    @if ($archivedCount > 0)
        <div
            x-data="{ showArchived: false }"
            class="mt-3 border-t border-gray-100 pt-3 dark:border-white/5"
        >
            <button
                type="button"
            ...
            >
                <span x-show="! showArchived">{{ __('candidates.materials.show_archived', ['count' => $archivedCount]) }}</span>
                    ...
                    <div class="border-b border-gray-100 py-3 first:pt-0 last:border-0 last:pb-0 dark:border-white/5">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <x-filament::button size="sm" color="primary" outlined wire:click="mountAction('restoreMaterial', { material: {{ $material['id'] }} })">
    ...
    @if (count($applications) > 0)
        <div class="mt-4 border-t border-gray-100 pt-3 dark:border-white/5">
            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('candidates.materials.application_documents') }}</p>
            <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">{{ __('candidates.materials.application_documents_description') }}</p>
