...
<div class="space-y-6">
        ...
        <x-filament::section :heading="__('applications.admin.interviews.' . $section . '_heading')" icon="heroicon-o-calendar-days">
            <div class="space-y-4">
                @forelse ($interviews[$section] as $interview)
                    <article
                        class="rounded-2xl border border-gray-200 bg-gray-50/70 p-4 sm:p-5 dark:border-white/10 dark:bg-white/5">
                        ...
                        @if ($interview['sync_error'])
                            <p
                                class="mt-4 rounded-xl border border-danger-200 bg-danger-50 px-3 py-2 text-sm text-danger-700 dark:border-danger-400/30 dark:bg-danger-500/10 dark:text-danger-300">
                                {{ __('applications.admin.interviews.sync_failed_description') }}
                            </p>
                        @endif

                        <dl
                            class="mt-4 grid gap-4 border-y border-gray-200 py-4 text-sm sm:grid-cols-2 xl:grid-cols-4 dark:border-white/10">
                            <div>
                                <dt
                                    class="text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">
                                    {{ __('applications.admin.interviews.fields.duration') }}</dt>
                                <dd class="mt-1 font-medium text-gray-950 dark:text-white">{{ $interview['duration'] }}
                                    {{ __('applications.admin.interviews.minutes_short') }}</dd>
                            </div>
                            <div>
                                <dt
                                    class="text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">
                                    {{ __('applications.admin.interviews.rsvp.label') }}</dt>
                                <dd class="mt-1 font-medium text-gray-950 dark:text-white">
                                    {{ __('applications.admin.interviews.rsvp.' . $interview['rsvp_status']) }}</dd>
                            </div>
                            <div>
                                <dt
                                    class="text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">
                                    {{ __('applications.admin.interviews.fields.last_synced_at') }}</dt>
                                <dd class="mt-1 font-medium text-gray-950 dark:text-white">
                                    {{ $interview['last_synced_at'] ?? __('applications.admin.interviews.not_synced') }}
                                </dd>
                            </div>
                            @if ($interview['cancelled_at'])
                                <div>
                                    <dt
                                        class="text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">
                                        {{ __('applications.admin.interviews.fields.cancelled_at') }}</dt>
                                    <dd class="mt-1 font-medium text-gray-950 dark:text-white">
                                        {{ $interview['cancelled_at'] }}</dd>
                                </div>
                            @endif
                        </dl>
                        ...
                        @if ($interview['feedback'] !== [])
                            <div class="mt-4 space-y-3">
                                ...
                                @foreach ($interview['feedback'] as $submission)
                                    @include(
                                        'filament.resources.applications.components.interview-feedback-submission',
                                        [
                        ...
                        @if (
                            $interview['meeting_url'] ||
                                $interview['can_reschedule'] ||
                                $interview['can_cancel'] ||
                                $interview['can_refresh'] ||
                                $interview['can_record_feedback']
                        )
                            <div class="mt-4 flex flex-wrap gap-2">
                                ...
                                @if ($interview['can_record_feedback'])
                                    <x-filament::button size="sm" color="primary" outlined
                                        wire:click="mountAction('recordInterviewFeedback', { interview: {{ $interview['id'] }} })">
                                        {{ __(
                                            $interview['has_own_feedback']
                                ...
                                @if ($interview['meeting_url'])
                                    <x-filament::button tag="a" size="sm" :href="$interview['meeting_url']" target="_blank"
                ...
                @empty
                    <x-filament::empty-state :contained="false" :heading="__('applications.admin.interviews.empty_' . $section)" icon="heroicon-o-calendar-days"
                        icon-color="gray" />
                @endforelse
            </div>
        </x-filament::section>
    ...
    <x-filament::section :heading="__('applications.admin.interviews.evidence.heading')" :description="__('applications.admin.interviews.evidence.description')" icon="heroicon-o-chat-bubble-left-right">
        @include('filament.resources.applications.components.interview-evidence', [
            'evidence' => $interviews['evidence'],
        ])
    </x-filament::section>

    <x-filament::section :heading="__('applications.admin.interviews.brief.heading')" :description="__('applications.admin.interviews.brief.description')" icon="heroicon-o-document-magnifying-glass">
        ...
        <div class="space-y-4">
            @forelse ($interviews['brief_items'] as $briefItem)
                <article
                    class="rounded-xl border border-primary-200 bg-primary-50/60 p-4 dark:border-primary-400/20 dark:bg-primary-500/10">
                    <div class="flex flex-wrap items-start justify-between gap-2">
                        <h3 class="font-semibold text-gray-950 dark:text-white">{{ $briefItem['criterion'] }}</h3>
                        <x-filament::badge :color="match ($briefItem['priority']) {
                            'high' => 'danger',
                            'medium' => 'warning',
                            default => 'gray',
                        }">
                            {{ __('applications.admin.interviews.brief.priority_label') }}
                            {{ __('applications.admin.interviews.brief.priority.' . $briefItem['priority']) }}
                        </x-filament::badge>
                    </div>
                    ...
                    <p
                        class="mt-3 rounded-lg bg-white/70 p-3 text-sm font-medium text-gray-950 dark:bg-black/10 dark:text-white">
                        <span class="font-semibold">
                            {{ __('applications.admin.interviews.brief.question_label') }} :
                        </span>
                        {{ $briefItem['question'] }}
                    </p>
                </article>
            @empty
                <x-filament::empty-state :contained="false" :heading="__('applications.admin.interviews.brief.empty')"
                    icon="heroicon-o-document-magnifying-glass" icon-color="gray" />
            @endforelse
        </div>
    </x-filament::section>
</div>
