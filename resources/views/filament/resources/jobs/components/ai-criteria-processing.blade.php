<div
    ...
    <div class="relative mx-auto flex max-w-2xl flex-col items-center gap-5 text-center">
        <div
            class="flex size-14 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-primary-200 dark:bg-gray-900 dark:ring-primary-700">
            <x-filament::loading-indicator class="size-7 text-primary-600 dark:text-primary-400" />
        </div>

        <div class="flex flex-col gap-2">
            <h2 class="text-lg font-semibold text-gray-950 dark:text-white">
                {{ __('jobs.criteria.processing_title') }}
            </h2>

            <p class="text-sm leading-6 text-gray-600 dark:text-gray-300">
                {{ __('jobs.criteria.processing_description') }}
            </p>
        </div>

        <div
            class="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-xs font-medium text-primary-700 shadow-sm ring-1 ring-primary-200 dark:bg-gray-900 dark:text-primary-300 dark:ring-primary-700">
            <span class="size-1.5 animate-pulse rounded-full bg-primary-500"></span>
            {{ $status === \App\Enums\JobCriteriaProcessingStatus::Processing
                ? __('jobs.criteria.status_processing')
                : __('jobs.criteria.status_pending') }}
        </div>
    </div>
</div>
