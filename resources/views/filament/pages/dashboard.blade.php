<x-filament-panels::page>
    <div class="grid gap-6">
        <section
            class="relative isolate overflow-hidden rounded-3xl bg-linear-to-br from-primary-700 via-primary-600 to-cyan-500 px-6 py-8 text-white shadow-xl shadow-primary-900/15 sm:px-8 sm:py-10 lg:px-12 lg:py-12">
            ...
            <div class="grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_18rem] lg:gap-12">
                <div class="max-w-3xl">
                    <div class="mb-6 flex flex-wrap items-center gap-3">
                        <span
                            class="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-sm font-medium backdrop-blur-sm">
                    ...
                    <h1 class="mt-3 text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">
                        <span data-clock-greeting data-good-morning="{{ __('dashboard.welcome.good_morning') }}"
                            data-good-afternoon="{{ __('dashboard.welcome.good_afternoon') }}"
                            data-good-evening="{{ __('dashboard.welcome.good_evening') }}">{{ $greeting }}</span>,
                        {{ $userFirstName }}!
                    </h1>
                ...
                <div class="relative mx-auto flex size-48 items-center justify-center sm:size-56 lg:size-64">
                    ...
                    <div
                        class="relative flex size-32 items-center justify-center rounded-3xl border border-white/30 bg-white/15 shadow-2xl backdrop-blur-md sm:size-36">
                        <x-filament::icon icon="heroicon-o-briefcase" class="size-20 text-white sm:size-24" />
                        <span
                            class="absolute -top-3 -right-3 flex size-12 items-center justify-center rounded-2xl bg-white text-primary-600 shadow-lg">
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                <article
                    ...
                    <div class="flex items-start gap-4">
                        <span
                            class="flex size-11 shrink-0 items-center justify-center rounded-xl bg-primary-50 text-primary-600 dark:bg-primary-500/10 dark:text-primary-400">
                ...
                <article
                    ...
                    <div class="flex items-start gap-4">
                        <span
                            class="flex size-11 shrink-0 items-center justify-center rounded-xl bg-violet-50 text-violet-600 dark:bg-violet-500/10 dark:text-violet-400">
                ...
                <article
                    class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm md:col-span-2 xl:col-span-1 dark:border-white/10 dark:bg-gray-900">
                    <welcome-clock class="block" data-locale="{{ str_replace('_', '-', app()->getLocale()) }}">
                        <div class="flex items-start gap-4">
                            <span
                                class="flex size-11 shrink-0 items-center justify-center rounded-xl bg-cyan-50 text-cyan-600 dark:bg-cyan-500/10 dark:text-cyan-400">
                            ...
                            <div class="min-w-0">
                                ...
                                <time data-clock-time datetime="{{ now()->toIso8601String() }}"
                                    class="mt-1 block text-2xl font-semibold tracking-tight text-gray-950 tabular-nums dark:text-white">{{ now()->format('H:i:s') }}</time>
                                <p data-clock-date class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    {{ now()->translatedFormat('l, d F Y') }}
                                </p>
                                <p data-clock-timezone
