        ...
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <h3 id="scoring-weights-heading" class="text-lg font-semibold text-gray-950 dark:text-white">
                    {{ __('settings.scoring.weights_heading') }}
                </h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    {{ __('settings.scoring.weights_description') }}
                </p>
            </div>
            <x-filament::button color="primary" wire:click="mountAction('updateScoringWeights')"
                wire:loading.attr="disabled">
                {{ __('settings.scoring.update.action') }}
            </x-filament::button>
        </div>

        <div class="mt-5 flex flex-wrap gap-2">
            <x-filament::badge color="primary" size="lg">
                {{ __('settings.fields.analysis_weight') }}: {{ $settings['analysis_weight'] }}%
            </x-filament::badge>
            <x-filament::badge color="gray" size="lg">
                {{ __('settings.fields.referral_weight') }}: {{ $settings['referral_weight'] }}%
            </x-filament::badge>
        </div>
    </section>
</div>
