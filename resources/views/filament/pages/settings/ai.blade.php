...
<div class="grid gap-6" data-testid="ai-settings">
        ...
        <div class="mt-5 grid gap-4 lg:grid-cols-2">
            <article
                @class([
                    ...
                    'border-primary-500 bg-primary-50/50 ring-2 ring-primary-500/10 dark:border-primary-400 dark:bg-primary-500/5' => $settings['provider'] === 'platform',
                    'border-gray-200 bg-white dark:border-white/10 dark:bg-gray-950/30' => $settings['provider'] !== 'platform',
                ])
            >
                <div class="flex items-start justify-between gap-4">
                    <span class="flex size-11 items-center justify-center rounded-2xl bg-primary-100 text-primary-700 dark:bg-primary-500/15 dark:text-primary-300">
                ...
                <div class="mt-5">
                    <x-filament::button
                        color="gray"
                        wire:click="mountAction('usePlatformAi')"
                        wire:loading.attr="disabled"
                        :disabled="$settings['configured_provider'] === 'platform'"
                    >
            ...
            <article
                @class([
                    'relative rounded-2xl border p-5 transition',
                    'border-primary-500 bg-primary-50/50 ring-2 ring-primary-500/10 dark:border-primary-400 dark:bg-primary-500/5' => $settings['provider'] === 'own',
                    'border-gray-200 bg-white dark:border-white/10 dark:bg-gray-950/30' => $settings['provider'] !== 'own',
                    'opacity-75' => ! $settings['own_key_allowed'],
                ])
            >
                <div class="flex items-start justify-between gap-4">
                    <span class="flex size-11 items-center justify-center rounded-2xl bg-violet-100 text-violet-700 dark:bg-violet-500/15 dark:text-violet-300">
                        <x-filament::icon icon="heroicon-o-key" class="size-6" />
                    </span>
                    @if ($settings['provider'] === 'own')
                        <x-filament::badge color="success" size="sm">{{ __('settings.ai.status.active') }}</x-filament::badge>
                ...
                @if ($settings['has_own_key'])
                    <div class="mt-4 flex flex-wrap items-center gap-2 rounded-xl bg-gray-50 px-3 py-2.5 dark:bg-white/5">
                        <code class="text-sm font-semibold text-gray-700 dark:text-gray-200">{{ $settings['masked_key'] }}</code>
                ...
                <div class="mt-5 flex flex-wrap gap-2">
                    @if ($settings['own_key_allowed'])
                        <x-filament::button
                            color="primary"
                            wire:click="mountAction('configureOwnAi')"
                            wire:loading.attr="disabled"
                        >
    ...
    <section class="rl-settings-panel" aria-labelledby="ai-usage-heading" data-testid="ai-usage">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                <x-filament::badge color="gray">{{ __('settings.ai.usage.platform') }}: {{ $settings['usage']['platform_used'] }}</x-filament::badge>
                <x-filament::badge color="gray">{{ __('settings.ai.usage.own') }}: {{ $settings['usage']['own_used'] }}</x-filament::badge>
            </div>
        </div>
        ...
        <div class="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <div>
                ...
                <progress
                    class="rl-progress-track mt-3 h-3"
                    max="100"
                    value="{{ $settings['usage']['bar_percentage'] }}"
                    aria-label="{{ $settings['usage']['percentage_label'] }}"
                ></progress>
            ...
            <div class="grid grid-cols-2 gap-3 lg:min-w-64">
                <div class="rounded-xl bg-gray-50 p-4 dark:bg-white/5">
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('settings.ai.current_provider') }}</p>
                    <p class="mt-1 text-sm font-semibold text-gray-950 dark:text-white">{{ $settings['provider_label'] }}</p>
                </div>
                <div class="rounded-xl bg-gray-50 p-4 dark:bg-white/5">
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('settings.fields.model') }}</p>
                    <p class="mt-1 text-sm font-semibold text-gray-950 dark:text-white">{{ $settings['model'] }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="rl-settings-panel" aria-labelledby="ai-history-heading" data-testid="ai-usage-history">
        ...
        @if ($settings['history'] === [])
            <div class="mt-6 flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-gray-50/70 px-6 py-10 text-center dark:border-white/15 dark:bg-white/5">
                ...
                <h4 class="mt-4 font-semibold text-gray-950 dark:text-white">{{ __('settings.ai.history.empty_heading') }}</h4>
                        ...
                        @foreach ($settings['history'] as $record)
                            <tr class="text-gray-700 dark:text-gray-200">
                                ...
                                <td class="whitespace-nowrap px-4 py-3">
                                    <x-filament::badge :color="$record['status_color']" size="sm">{{ $record['status'] }}</x-filament::badge>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
